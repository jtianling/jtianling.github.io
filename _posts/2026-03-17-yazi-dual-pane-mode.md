...
最简单的方式是通过我自己的 Homebrew tap 装:

```bash
brew install jtianling/tap/dual-yazi
```

或者先 `brew tap jtianling/tap` 再 `brew install dual-yazi`, 效果一样. macOS (Apple Silicon / Intel) 和 Linuxbrew 都能装. 暂时没做预编译 bottle, 第一次装会从源码编译 Rust 项目, 我自己的机器上大概 5-10 分钟. 升级走 `brew upgrade dual-yazi` 就行.

有一点需要注意: `dual-yazi` 装出来的 binary 仍然叫 `yazi` 和 `ya`, 跟原版同名, formula 里也声明了 `conflicts_with "yazi"`. 好处是上游的 keymap, plugin, 文档里所有引用 `yazi` 命令的地方都能直接复用; 代价是不能跟原版共存. 已经装了官方 Yazi 的话, 先:
...
如果你不想走 brew, 或者已经习惯从源码装 Rust 项目, 也可以直接 build:
