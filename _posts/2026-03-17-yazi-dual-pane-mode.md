...
但真动手之后发现, Yazi 的 plugin 在能力边界上还是有比较明显的限制. 像固定双栏布局、每栏独立的 tabs 和 cursor 状态、跨栏的 `F5` / `F6` 这种一键复制/移动, 涉及到渲染层和核心状态结构的改动, plugin 层其实没有那么大的权限去干预 —— 至少不足以把我想要的这套完整双栏工作流表达出来. 硬要绕的话, 要么功能上要打折, 要么实现上非常 hacky.

所以最后还是选择了 fork 的方式, 直接在源码里把双栏作为一等公民加进去. 代价是需要自己维护和主线的同步, 但换来的是可以把每一块交互都做到我想要的样子. 对我来说, 这个取舍是值得的.
