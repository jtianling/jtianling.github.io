...
启 daemon (起一次, 让它持续跑, 单独终端 / `tmux` / `launchd` 都行):

```bash
npx -y cross-agent-teams-mcp@latest daemon --port 9100
```

然后给 Claude Code 的 `.mcp.json` 加两个条目, 一个是 HTTP 入口, 一个是 channel shim:
...
启动 Claude Code 时带上 channel loader:
...
> 把我注册到 xats, 名字叫 alice.
>
> 给 bob 发个消息: 迁移那个事情进度怎么样了?
>
> 我邮箱里有什么新消息?
>
> xats 上现在还有谁在?
