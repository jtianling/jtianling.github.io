...
分享两个C++模板函数，简化容器打印。支持输出整个容器或指定范围，并允许添加前缀说明。
...
竟然是要简单，所以当然应该提供只需要一个容器参数就可以输出的方法，但是却还想要保留输出范围的能力，怎么办呢？重载。。。。另外，输出的时候前面一般加说明，这里以一个默认为空的参数加进来，这样更加方便了，另外，我用const std::string而不用const char*是为了应用范围更广泛，因为有char* 到 string的默认构造函数，可以自动转换，反之则不行，不知道这样有没有副作用，高手提醒之。还有，因为常用，我把它放到myself名字空间下。这里未与列出。
