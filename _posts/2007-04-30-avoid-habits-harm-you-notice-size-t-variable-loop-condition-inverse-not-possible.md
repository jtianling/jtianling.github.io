...
本文分享了C++编程中的一个陷阱：使用无符号的size_t类型做递减循环，因其值不会小于零而导致死循环，并分享了修正方法。
...
首先声明的是，我是在学习用数组处理这个问题，所以没有用vector，而是动态分配了数组，求位数用的是（n!=10^M）估算算法，每一位只保存结果的一位，为了节省空间用unsigned char保存一位数，在在vc2005下编译通过了，不过运行却得不出结果，还有假如在最后把输出的for(size_t i = am;i >= 0;--i)改为 for(int i = am;i >= 0;--i)，结果会好一点，但是还是得不出正确结果，正准备找高手说明下问题。还有用size_t为什么似乎会导致这里的死循环，而int不会？最后我发现，把print（）定义成这个样子，反而又可以。所以比较肯定的说是输出函数的问题，但是我实在不知道是什么问题。
